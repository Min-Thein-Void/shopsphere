<template>
    <div class="w-full bg-gray-900 px-6 py-6 space-y-6 rounded-lg">
        <!-- Profile Section -->
        <div class="flex flex-col items-center text-center">
            <label class="relative cursor-pointer group">
                <img :src="userStore.user && userStore.user.avatar ? userStore.user.avatar : '/public/robot.png'"
                    alt="Profile"
                    class="w-16 h-16 rounded-full border border-green-500 shadow-md group-hover:opacity-80 transition" />
        ...
        <nav class="space-y-2 text-sm font-medium text-white">
            <router-link :to="{ name: 'Home' }"
                class="flex items-center gap-3 px-2 py-2 rounded hover:bg-gray-700 transition">
                <!-- Home Icon -->
                <svg class="w-4 h-4 text-gray-300 group-hover:text-green-400" fill="none" viewBox="0 0 24 24"
                    stroke="currentColor">
            ...
            <div class="flex items-center gap-3 px-2 py-2 rounded hover:bg-gray-700 transition cursor-pointer">
                <!-- Bell Icon -->
                <svg class="w-4 h-4 text-gray-300 group-hover:text-green-400" fill="none" viewBox="0 0 24 24"
                    stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
            ...
            <button v-if="userStore.user !== null" @click="Logout"
                class="flex items-center gap-3 px-2 py-2 rounded text-red-400 hover:text-red-600 transition w-full">
                <!-- Logout Icon -->
...
export default {
    setup() {
        ...
        let uploadProfilePic = async (event) => {
            ...
            try {
                let res = await api.post(`/api/auth/user/avatar/${userStore.user.id}`, formData, {
                    headers: {
